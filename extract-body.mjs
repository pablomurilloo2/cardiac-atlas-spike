/**
 * Extrae el sistema cardiovascular de cuerpo completo de human-atlas
 * (BodyParts3D 4.0, CC BY 4.0): arterial + venoso + corazón, más la piel
 * como silueta de orientación.
 *
 * Uso:  node extract-body.mjs <ruta-a-human-atlas> <dir-salida>
 */
...
if (!atlasRoot) { console.error('uso: node extract-body.mjs <human-atlas> [out]'); process.exit(1); }

const BRAIN_FMA = new Set(['FMA78449', 'FMA78450', 'FMA78469', 'FMA78454', 'FMA75351']);
const SYSTEMS = new Set(['arterial', 'venous', 'cardiac', 'integumentary']);
...
const wanted = atlas.parts.filter((p) =>
  SYSTEMS.has(p.system) && !(p.system === 'cardiac' && BRAIN_FMA.has(p.conceptId)));
...
writeFileSync(join(outDir, 'body-subset.bin'), bin);
writeFileSync(join(outDir, 'body-subset.json'), JSON.stringify({
...
console.log(`mallas: ${parts.length}`, by);
console.log(`triángulos: ${totalTri.toLocaleString()}`);
console.log(`binario: ${(bin.length / 1e6).toFixed(1)} MB · gzip: ${(gzipSync(bin, { level: 6 }).length / 1e6).toFixed(1)} MB`);
